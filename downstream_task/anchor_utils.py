...
class Anchors(nn.Module):
    def __init__(self,
        ...
        # FPN levels: must match your PyramidFeatures / RetinaNet head
        if pyramid_levels is None:
            self.pyramid_levels = [3, 4, 5, 6, 7]
        else:
            self.pyramid_levels = pyramid_levels

        # Strides for each pyramid level (in pixels)
        if strides is None:
            self.strides = [2 ** x for x in self.pyramid_levels]  # [8, 16, 32, 64, 128]
        else:
            self.strides = strides

        # >>> Anchor sizes tuned for full-resolution (no extra resize) <<<
        # Original implicit formula: [2 ** (x+2) for x in [3..7]] -> [32, 64, 128, 256, 512]
        if sizes is None:
            self.sizes = [32, 64, 128, 256, 512]
        else:
            self.sizes = sizes

        # Aspect ratios and scales per level
        if ratios is None:
